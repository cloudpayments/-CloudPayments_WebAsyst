 ...
 * @property string $shipping_spic
 * @property string $shipping_package_code
 */
  ...
  public function payment($payment_form_data, $order_data, $auto_submit = false)
  {
    ...
    foreach ($order->items as $single_item) {
      ...
      $total_discount = (float)str_replace(
        ...
        $product = new shopProduct($single_item["product_id"]);
        $product_features = $product->getFeatures();

        if (key_exists('packageCode', $product_features) && key_exists('spic', $product_features)) {
            $cp_item['packageCode'] = $product_features['packageCode'];
            $cp_item['spic'] = $product_features['spic'];
            if (!key_exists('AdditionalReceiptInfos', $hidden_fields)) {
                $hidden_fields['AdditionalReceiptInfos'] = ["Вы стали обладателем права на 1% cashback"]; // Это статичное значение
            }
        }
    ...
    if ($shipping > 0) {
      ...
      $shipping_spic = $this->shipping_spic;
      $shipping_package_code = $this->shipping_package_code;

      if ($shipping_spic && $shipping_package_code) {
        $cp_item['spic'] = $shipping_spic;
        $cp_item['packageCode'] = $shipping_package_code;
      }
    ...
    $view->assign('widget_url', 'https://widget.cloudpayments.' . $this->widgetCountry ?: 'ru');
